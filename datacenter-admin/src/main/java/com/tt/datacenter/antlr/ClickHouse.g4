...
// 简单的select查询，from子句没有：子查询、join、union
select_simple:
    K_SELECT
            column_expr_list
        ( from_simple )
...
//简单查询的from子句
from_simple:
    K_FROM full_table_name ( K_AS? table_lias)
    ;

// 包含join的select
join_select:
    K_SELECT column_expr_list
    (join_from)
    ;
...
from_stat:
    K_FROM tableSources
    ( full_table_name | LPAREN? subquery RPAREN? ) ( K_AS? table_lias )?
    ( join_stat )?
...
join_type:
    K_GLOBAL?
    ( K_ANY | K_ALL | K_ASOF )?
    ( K_INNER | K_LEFT | K_RIGHT | K_FULL | K_CROSS )?
    ( K_OUTER | K_SEMI |K_ANTI )?
    K_JOIN
    ;
...
join_on_expr
    : (K_NOT | '!') join_on_expr
    | join_on_expr logicalOperator join_on_expr
    ;
logicalOperator
    : K_AND | K_OR
    ;
