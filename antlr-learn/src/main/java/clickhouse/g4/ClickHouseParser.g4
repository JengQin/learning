...
statement:
        select_simple_statement
//    |   insert_simple_statement
...
select_join_step
 :  K_GLOBAL?
        ( K_ANY | K_ALL | K_ASOF )? ( K_INNER | K_LEFT | K_RIGHT | K_FULL | K_CROSS )? ( K_OUTER | K_SEMI |K_ANTI )
        K_JOIN select_join_right_part
        ( K_ON LPAREN? LPAREN booleanExpression RPAREN)?
        ( K_USING LPAREN not_empty_expression_list RPAREN | K_USING not_empty_expression_list)
